...
///         (<c>Bimwright.Inventor.Plugin.InvNN.dll</c>).</item>
...
public sealed class AddinManifestTests
{
    ...
    {
        var nn = year - 2000;
        var path = Path.Combine(PluginDir(year), $"Bimwright.Inventor.Inv{nn:00}.addin");
    ...
    {
        var addin = LoadAddin(year);
        Assert.Equal($"Bimwright.Inventor.Plugin.Inv{year - 2000:00}.dll", Element(addin, "Assembly"));
    }
